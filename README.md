...
阶段一至阶段三功能和文档已经完成。当前 `phase/3-release` 包含 Next.js 应用、四个 Provider 的 Mock/live Adapter、独立思考/回答流、实际响应模型显示、非流式与 SSE 聊天 API、响应式聊天工作区、本地会话与提示词、长文档导入、生成参数、安全 Markdown、回复复制与会话导出，以及安全的 Live 密钥启动脚本。项目已完成依赖漏洞修复、API 安全回归和 CI；本次协议更新后的 Linux/amd64 镜像需重新构建并替换旧 `.tar`/SHA-256。
...
3. 阶段三：测试、依赖漏洞修复、测试报告和面向运维的部署手册已更新；协议更新后的 amd64 容器需重新构建收口。
