...
| 阶段三：交付 | 完成测试与漏洞修复；制作 amd64 镜像；提交测试记录、报告和部署手册 | 80 项测试、依赖审计 0、DeepSeek Live 烟囱测试、非 root amd64 镜像、镜像 tar/校验、运维流程 | `test-records.md`、`test-report.md`、`deployment-guide.md`、镜像 `.tar`/SHA-256 | 完成；公网增强项除外 |
...
| U-07 | 统一请求校验、timeout、AbortSignal 和安全错误 | 二、三 | 严格字段、Provider/模型、角色、长度、参数、60 秒 timeout、取消传播 | validation/chat-handler/security tests | 完成 |
...
| U-11 | temperature、max_tokens 和安全 Markdown | 二 | 0–2、1–8192；受控 React Markdown | chat-settings/safe-markdown tests | 完成 |
...
| U-14 | TXT、Markdown、JSON 本地文档 | 三 | 3 个文件、单个 100 KB、JSON 校验；正文发送后清除 | local-documents implementation/tests | 完成；不是向量检索或服务端知识库 |
...
| U-19 | 真实 DeepSeek 测试和 reasoning 兼容 | 三 | DeepSeek 鉴权、模型列表、非流式/流式；content 为空时处理 reasoning_content | OpenAI-compatible Adapter/tests、测试记录 | 完成；Max tokens 1024 已验证 |
...
| U-23 | 阶段三文件夹包含完整交付 | 三 | 69 个源码/文档文件、镜像 tar、SHA-256；排除 Git/依赖/缓存/密钥 | `D:\E2026.08.29\中国移动\ai coding\阶段三` | 完成 |
