...
describe("chat page settings", () => {
  it("passes the stage-one defaults to /api/chat request data", () => {
    expect(buildPageChatRequest("openai", "gpt-4o-mini", [{ role: "user", content: "hello" }], DEFAULT_TEMPERATURE, DEFAULT_MAX_TOKENS)).toEqual({
      provider: "openai",
      model: "gpt-4o-mini",
  ...
  it("accepts boundary values and rejects values outside the approved ranges", () => {
    ...
    expect(() => buildPageChatRequest("openai", "gpt-4o-mini", [{ role: "user", content: "hello" }], 3, 256)).toThrow(RangeError);
  });
});
